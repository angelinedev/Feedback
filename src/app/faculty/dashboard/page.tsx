...
export default function FacultyDashboard() {
  ...
  const feedbackForSubject = useMemo(() => {
    ...
    return mockFeedback.filter(f => f.faculty_id === selectedMapping.faculty_id && f.subject === selectedMapping.subject);
  }, [selectedMapping]);
  
  const totalStudentsInClass = useMemo(() => {
    if (!selectedMapping) return 0;
    return mockStudents.filter(s => s.class_name === selectedMapping.class_name).length;
  }, [selectedMapping]);
